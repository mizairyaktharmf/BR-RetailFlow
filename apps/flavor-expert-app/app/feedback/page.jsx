...
import { Loader2, Star, CheckCircle2, IceCream } from 'lucide-react'
import api from '@/services/api'

// ─── Root export wraps content in Suspense (required for static export) ───────
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
          ...
          <div className="flex gap-1 mt-6">
            {[1, 2, 3, 4, 5].map((s) => (
              <Star
                key={s}
                className={`w-7 h-7 ${s <= rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-200 fill-gray-200'}`}
              />
            ))}
          </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto">
        <form onSubmit={handleSubmit} className="max-w-lg mx-auto p-5 space-y-6 pb-10">
          ...
          <section className="bg-white rounded-2xl shadow-sm p-6">
            ...
            {rating > 0 && (
              <p className="text-center text-sm font-medium text-purple-600 mt-3">
                {['', 'Very Poor', 'Poor', 'Good', 'Very Good', 'Excellent'][rating]}
              </p>
            )}
          </section>
